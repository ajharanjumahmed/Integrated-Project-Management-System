    ...
    <DashboardLayout title="Projects">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-slate-400 text-sm mt-0.5">{{ projects.length }} total project{{ projects.length !== 1 ?
                    's' : '' }}</p>
            </div>
            ...
            <table class="w-full text-sm">
                ...
                <thead>
                    <tr class="border-b border-slate-100">
                        <th class="text-left px-6 py-3.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">
                            Project</th>
                        <th class="text-left px-6 py-3.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">
                            Client</th>
                        <th class="text-left px-6 py-3.5 text-xs font-semibold text-slate-400 uppercase tracking-wide">
                ...
                <tbody class="divide-y divide-slate-50">
                    <div v-if="!filtered().length" class="px-6 py-12 text-center text-slate-400 text-sm">
                        No projects match your filters.
                    </div>
                    <tr v-for="project in filtered()" :key="project.id" class="hover:bg-slate-50/60 transition">
                        <td class="px-6 py-3.5 font-medium text-slate-800">
                            <Link :href="`/projects/${project.id}`" class="hover:text-blue-600 transition">
                                {{ project.title }}
                            </Link>
                        </td>
                        ...
                        <td class="px-2 py-3.5 text-right font-semibold text-slate-700">
